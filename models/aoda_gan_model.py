...
import torch
from util.image_pool import IClassPool, ImagePool
...
class AODAGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:  # define discriminators
            ...
            # for feature loss, eta loss
            if self.opt.lambda_C > 0:
                self.netF = networks.define_F(self.gpu_id, use_bn=False)
                self.netF.to(self.device)
            if self.opt.lambda_E > 0:
                self.netR = networks.define_R(opt, self.gpu_id)
            ...
            self.criterionCycle = torch.nn.L1Loss() #? 故意的，还是漏了?
    ...
    def forward(self):
        ...
        if self.isTrain:
            if "aoda" in self.opt.name:  # adopt the random-mixed sampling strategy
                ...
                if (
                    p > self.opt.rand_p and self.eta.size(0) == self.opt.batch_size
                ):  # skip the last batch
    ...
    def backward_D_A(self):
        ...
        fake_B = self.fake_B_pool.query(self.fake_B)  #! modified
